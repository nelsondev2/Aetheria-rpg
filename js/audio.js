...
const AudioSys = (() => {
  ...
  const SFX = {
    ...
    slash: () => { const t = ctx.currentTime; playNoise(t, .08, .22, null, 2800); playTone(880, t, .07, 'square', .16, null, .5); playTone(1320, t+.03, .05, 'square', .1); },
    hit: () => { const t = ctx.currentTime; playNoise(t, .12, .35, null, 900); playTone(180, t, .1, 'sawtooth', .22, null, .5); },
    hurt: () => { const t = ctx.currentTime; playNoise(t, .16, .4, null, 500); playTone(140, t, .14, 'sawtooth', .28, null, .4); },
    dash: () => { const t = ctx.currentTime; playNoise(t, .1, .2, null, 1800); playTone(520, t, .08, 'square', .14, null, 1.8); },
